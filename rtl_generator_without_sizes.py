...
################# peremennie  ##################
    ...
    for i in vars_list:
        ...
        while(a != max_level_of_var):
            ...
            an_str = "register #(1,\"sync\",\"posedge\",0,0,32," + str(N)+ "\'d0)register_" + str(i)+str(a) + "(.i_clk(i_clk), .i_reset(i_reset), .i_EN(1'b1), .i_d(RF_"+ str(i)+str(a-1) + "), .o_d(RF_"+ str(i)+str(a) + "));\n"
            inf.write(s)
            inf.write(an_str)
            a += 1
    ...
    for k,v in per_dict.items():
        ...
        s_3 = 'wire ' + '[N-1:0] ' +k + str(level_of_var) + ';\n'

        lst = t_size - 1
        s_4 = 'register #(1,"sync","posedge",0,0,N,{N{1\'d0}}' + ')register_'+ k + str(level_of_var) 
        s_4 += '(.i_clk(i_clk), .i_reset(i_reset), .i_EN(1\'b0), .i_d(in_'+k+ str(level_of_var) + '[' + str(lst) + ':' + str(lst-N+1) +'])'
        s_4 += ', .o_d('+k+ str(level_of_var) +'));\n' 
    ...
    s_5 = 'register #(1,"sync","posedge",0,0,N,{N{1\'d0}}'  
    s_5 += ')register_RF_Res_'+ str(i-1)+'(.i_clk(i_clk), .i_reset(i_reset), .i_EN(1\'b0), .i_d(in_'
    s_5 += str(last_key)+ str(level_of_var) + '[N-1:0]), .o_d(RF_Res_'+ str(i-1)+'));\n'
    inf.write(s_5)
